...
const PetCard: React.FC<PetCardProps> = ({ 
  ...
  return (
    <div className={`pet-card ${!pet.isAvailable ? 'unavailable' : ''} ${className}`}>
      <div className="pet-image-container">
        <img 
          ...
          onError={(e) => {
            ...
            e.currentTarget.src = `data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="300" height="200" viewBox="0 0 300 200"><rect width="300" height="200" fill="%23f8f9fa"/><text x="150" y="100" text-anchor="middle" fill="%236c757d" font-size="48">${getSpeciesEmoji(pet.species)}</text></svg>`;
          }}
        />
        {!pet.isAvailable && (
        ...
        <div className="pet-badges">
          ...
          {pet.spayedNeutered && <span className="badge spayed">✅ Spayed/Neutered</span>}
        </div>
      </div>
      ...
      <div className="pet-info">
        ...
        <div className="pet-compatibility">
          {pet.goodWithKids && <span className="compatibility-icon" title="Good with kids">👶</span>}
          {pet.goodWithPets && <span className="compatibility-icon" title="Good with other pets">🐾</span>}
          <span className="energy-level" title={`Energy level: ${pet.energyLevel}`}>
            ⚡ {pet.energyLevel}
          </span>
        </div>
